...
useEffect(() => {
  ...
  return (
      <div className="w-full mx-auto pb-4 bg-slate-200 ">
          {/* Section 1 */}
          <div className=' w-5/6 mx-auto h-80  bg-gradient-to-r from-biru to-pink overflow-hidden'>
            {/* TimeStamps */}
              <div className=' border-8 border-cream rounded-3xl w-screen relative -rotate-12 h-screen -mt-96 -top-20  flex flex-col justify-end p-4  bg-hitam mx-auto mb-8'>
                <p className=' ml-20 text-hijau text-4xl'> SPE / FRONTEND </p>
                <p className=' text-hijau text-lg rotate-90 bottom-28 -left-20 absolute '> {date}</p>
        ...
        <div className=' w-5/6  mx-auto flex flex-col gap-8 bg-white justify-around items-center'>
         {/* Head */}
          ...
          {data ? data.map((x) => (
            ...
            <div key={x.code} className=' w-full flex flex-row justify-between px-4 items-center gap-8 border-b-2 border-zinc-300 pb-4 mt-4'>
              ...
              <img className='border p-2 shadow-lg rounded-lg border-zinc-300' width='250' src={x.media_url}/>
              ...
              <p className='text-md font-bold'>IDR. { "Rp. " + (Math.round(x.price * 100) / 100).toLocaleString()}</p>
              ...
              <p className=' mr-16'>{ x.subTotal ? "Rp. " + (Math.round(x.subTotal * 100) / 100).toLocaleString() :'Rp. '+ 0}</p>
